class Student:
    ...
    def teaching_assessment(self, lecturer, course, grade):
        if isinstance(lecturer,
                      Lecturer) and course in self.courses_in_progress and course and lecturer.courses_attached:
    ...
    def __str__(self):
        ...
        res = (f"\nИмя: {self.name}"
               ...
               f"\nСредняя оценка за домашние задания: {self.average_rating(grade)}"
               f"\nКурсы в процессе изучения: {', '.join(self.average_rating(list_courses_with_grade))}"
...
class Lecturer(Mentor):
    ...
    def average_feedback(self):
        ...
        for key, value in self.student_assessment.items():
            average_feedback_list += value
        mean = round((sum(average_feedback_list) / len(average_feedback_list)), 1) if len(
            average_feedback_list) != 0 else 0.0
        return mean
...
def average_feedback_lectures(course, *lectures):
    ...
    for lecturer in lectures:
        if lecturer.student_assessment.get(course):
            list_st.extend(lecturer.student_assessment.get(course))
    return round(sum(list_st) / len(list_st), 1)
...
print(f"\n{'=' * 80}\n'Задание № 4.'")
course = "GIT"
print(f'Ср. балл студентов за курс {course}: {average_student_grade(course, academically_successful_student_1, academically_successful_student_2)}')
print(f'Ср. балл лекторов за курс {course}: {average_feedback_lectures(course, lector_course_1, lector_course_2)}')
